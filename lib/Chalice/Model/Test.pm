...
sub test_cm {
    ...
    my $title_collision;
    ok eval { $title_collision = $m->create_post(%data) },
       'Can create a post with the same title as an existing title';
    cmp_ok $title_collision->url, 'ne', $url, '... and it creates a new URL';
    diag("new url: " . $title_collision->url);
    $title_collision->delete;
